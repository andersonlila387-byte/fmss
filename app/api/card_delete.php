...
try {
    ...
    $stmt = $db->prepare("SELECT * FROM cards WHERE id = :id AND owner_id = :owner");
    ...
    $del = $db->prepare("DELETE FROM cards WHERE id = :id AND owner_id = :owner");
    $del->execute(['id' => $id, 'owner' => $user['id']]);
    
    $payloadHash = hash('sha256', $card['encrypted_payload']);
    \FMSS\Services\LedgerService::recordAction('CARD_DELETED', $user['id'], $payloadHash, ['card_id' => $id]);
