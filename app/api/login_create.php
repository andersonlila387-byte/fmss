...
try {
    ...
    $stmt = $db->prepare("
        INSERT INTO logins (id, owner_id, title, encrypted_payload, encrypted_data_key, iv)
    ...
    // Write to Ledger (Use the hash of the payload as the target_hash)
    $payloadHash = hash('sha256', $encryptedPayload);
    \FMSS\Services\LedgerService::recordAction('LOGIN_CREATED', $user['id'], $payloadHash, ['login_id' => $id, 'title' => $title]);
